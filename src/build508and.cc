...
void check_memory() {
    while (!block_memory_check) {
        ...
        double totalRAM = static_cast<double>(memInfo.ullTotalPhys) / (1024 * 1024);
        double availRAM = static_cast<double>(memInfo.ullAvailPhys) / (1024 * 1024);
#else
        struct sysinfo memInfo;
        if (sysinfo(&memInfo) != 0) {
            cerr << "Failed to get memory status." << el;
        }
        double totalRAM = static_cast<double>(memInfo.totalram) * memInfo.mem_unit / (1024 * 1024);
        double availRAM = static_cast<double>(memInfo.freeram) * memInfo.mem_unit / (1024 * 1024);
#endif
        double usedRAM = totalRAM - availRAM;
        double usagePercent = totalRAM > 0 ? (usedRAM / totalRAM) * 100.0 : 0.0;
...
void detect_forced_shutdown(int signal) {
    ...
    FILE* flag = fopen(CRASH_FLAG_PATH.c_str(), "w");
    if (flag) {
...
const string CRASH_FLAG_PATH = "D:/Console AndroidOS/bin/crash.flag";
const string GENUINE_PATH = "D:/Console AndroidOS/bin/genuine.txt";
#else
const string KEYS_PATH = "src/keys/keys.txt";
const string ACT_PATH  = "src/keys/activated.flag";
const string RESET_FLAG_PATH = "src/keys/reset.flag";
const string CRASH_FLAG_PATH = "bin/crash.flag";
const string GENUINE_PATH = "bin/genuine.txt";
#endif
...
void task_manager()
{
    ...
    struct sysinfo memInfo;
    if (sysinfo(&memInfo) == 0) {
        double totalMB = static_cast<double>(memInfo.totalram) * memInfo.mem_unit / (1024 * 1024);
        double availMB = static_cast<double>(memInfo.freeram) * memInfo.mem_unit / (1024 * 1024);
        double usedMB = totalMB - availMB;
        double usagePercent = totalMB > 0 ? (usedMB / totalMB) * 100.0 : 0.0;
        cout << "Memory Usage: " << fixed << setprecision(1) << usedMB << " MB used / " << totalMB << " MB total (" << usagePercent << "%)\n";
    }
#endif
}
...
void system_benchmark()
{
    ...
    cout << "CPU Cores: " << (cpuCount ? cpuCount : 1) << endl;

    // Read /proc/cpuinfo for frequency information if available.
    ...
    cout << "RAM Speed: Unknown on Unix without privileged access" << endl;
#endif
}
...
static bool read_genuine_state(const string &path = GENUINE_PATH) {
    ifstream gf(path);
...
#else
        system("python3 src/sounddriver.py src/windows_xp_error.ogg");
#endif
    }
}
...
#else
    string command = "python3 src/sounddriver.py \"" + soundinp + "\"";
#endif
...
void newui() {
    if (activated) {
        ...
        while (true) {
            ...
            struct sysinfo memInfo;
            if (sysinfo(&memInfo) == 0) {
                availMB = static_cast<double>(memInfo.freeram) * memInfo.mem_unit / (1024 * 1024);
            }
#endif
...
bool isProcessRunning(const std::wstring& processName) {
    ...
    if (Process32First(snapshot, &info)) {
        do {
            ...
            std::wstring exeWstr(info.szExeFile);
...
int main()
{
    ...
    system("title main");
#else
    // no-op on Unix: keep terminal title if already set by shell
#endif
