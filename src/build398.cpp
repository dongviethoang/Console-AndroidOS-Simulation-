...
void bootloader()
{
    ...
    this_thread::sleep_for(chrono::seconds(1));
    cout << "Build 398 (Kernel Version 25)" << endl;
    this_thread::sleep_for(chrono::seconds(1));
...
ofstream writeFile("newdisk.nvme", ios::out | ios::trunc);

void update()
{
    bool build_avail = true;
    cout << "Updating to build: 420" << endl;
    if (build_avail == false)
    {
    ...
    else
    {
        sleep.delay(2000);
        cout.flush();
        for (int pct = 0; pct <= 248; ++pct) {
            cout << "\rInstalling... " << setw(3) << pct << "KB " << flush;
            this_thread::sleep_for(chrono::milliseconds(15));
        }
        writeFile << "space=2048MB" << el;
        writeFile << "setchar=D" << el;
        writeFile << "data=oldsys";
        writeFile.close();
        system("build420.exe");
    }
}
...
int main()
{
    ...
    if (update_available)
    {
        cout << "Build 398 has reached it's EOL. Please update to build 420 to get the lastest features." << el;
        cout << "Stay in B398 at your own risk." << el;
    }
    bootloader();
...
g++ -std=c++23 build398.cpp -o "D:\Console AndroidOS\bin\build398.exe"

Yes, Build 398 uses C++23 (i might use C++26 in the future)
